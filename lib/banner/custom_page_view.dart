...
/// the [PageView], a [CustomPageController] also lets you control the offset in terms
...
/// to animate the [PageView] using a [CustomPageController].
///
/// ```dart
...
class CustomPageController extends ScrollController {
  ...
  /// TODO fix: 新增一个变量，控制滑动到拼接的页面时是否自动切换到真实的页面
  bool updatePage;
  ...
  ///
  /// 1. No [PageView] is currently using this [CustomPageController]. Once a
  /// [PageView] starts using this [CustomPageController], the new [page]
  ...
  ///
  /// 2. More than one [PageView] using the same [CustomPageController].
  ///
    ...
    return NotificationListener<ScrollNotification>(
      onNotification: (ScrollNotification notification) {
        ...
        if (notification.depth == 0 && widget.onPageChanged != null && notification is ScrollUpdateNotification) {
          ...
          final int currentPage = metrics.page!.round();
          widget.controller.updatePage = false;

          /// INFO 这里会回调多次，用于指示器更新
          widget.onPageChanged!(currentPage);
        } else if (notification.depth == 0 && widget.onPageChanged != null && notification is ScrollEndNotification) {
          final CustomPageMetrics metrics = notification.metrics as CustomPageMetrics;
          final int currentPage = metrics.page!.round();
          if (currentPage != _lastReportedPage) {
            _lastReportedPage = currentPage;
            widget.controller.updatePage = true;

            /// INFO 这里回调一次，用于控制滑动到拼接的页面时自动切换到真实的页面
            widget.onPageChanged!(currentPage);
          }
        }
        return false;
